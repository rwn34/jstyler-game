...
var APP_VERSION = 'v1.2.44'; // Build version — updated by zipgame.ps1
...
function loadSync(username,mmyy,pin,callback){
    if(!METRIC_URL)return;
...
function showPwaInstallBanner(){
    ...
    var shouldShow=pwaSupported&&!inStandalone&&installedAt===0&&!rewardClaimed;
    var isLandscapeNarrow=(W.matchMedia&&W.matchMedia('(orientation: landscape) and (max-height: 600px)').matches);
    if(banner)banner.style.display=(shouldShow&&!isLandscapeNarrow&&deferredPrompt)?'flex':'none';
